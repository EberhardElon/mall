<template>
  <div>
            ...
            <ul class="cart-item-list">
              <li v-for="item in cartlist">
                <div class="cart-tab-1">
                  <div class="cart-item-check" @click="editCart('check',item)">
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="delCartShow(item)">
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a class="btn btn--red" :class="{'btn--dis':productCount==0}" @click="checkout">Checkout</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <modal v-show="delCartConfirm" :md-show="delCartConfirm" @close="delCartConfirm=false">
      <p slot="message">
        你确认要删除这个商品吗？
      </p>
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="delCart">确认</a>
        <a class="btn btn--m" href="javascript:;" @click="delCartConfirm = false">关闭</a>
      </div>
    </modal>
    ...
    computed: {
      checkAllFlag() {
        if(this.productCount>0){
          return this.productCount == this.cartlist.length ? '1' : '0'
        }else{
          return '0';
        }
      },
    ...
    methods: {
      ...
      delCart() {
        axios.post("/users/delCart", {
          ...
          var res = response.data;
          if (res.status == '0') {
            this.delCartConfirm = false;
            this.getCartlist();
            this.$store.commit("updateCartCount",-this.product.productNum);
          }
        });
      },
      delCartShow(item) {
        this.product = item;
        this.delCartConfirm = true;
      },
      editCart(flag, item) {
        var num=0;
        switch (flag) {
